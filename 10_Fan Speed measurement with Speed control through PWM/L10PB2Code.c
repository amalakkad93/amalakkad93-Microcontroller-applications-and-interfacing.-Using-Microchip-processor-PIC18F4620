...
void main(void)
{
    ...
    while(1)
    {
        volt = read_volt();                 //get voltage reading
        duty_cycle = get_duty_cycle(volt);  //generate dc based on voltage
        duty_cycle = get_duty_cycle();
        do_update_pwm(duty_cycle) ;         //Update pwm for dc
        rpm = get_RPM();                    //Get rotations per minute
        printf ("duty cycle = %d RPM = %d\r\n",duty_cycle, rpm);//Display dc ad rpm on teraterm
