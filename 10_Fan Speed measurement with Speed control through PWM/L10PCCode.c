...
#define FAN_EN   PORTAbits.RA5              // Defines FAN ENABLE LED
...
int FANEN;                                  //Store value of FANEN
...
void interrupt high_priority chkisr()       //Set priority of the interrupts
{
    ...
    if (INTCON3bits.INT1IF == 1) INT1_ISR();//Go to INT1_ISR
}
...
void main(void)
{
    ...
    while(1)
    {
        ...
        {
            printf("The fan is off\n");     //Print fan off in teraterm the fan is off
        ...
        {
            printf("The fan is on\n");      //Print fan on in teraterm
            Turn_On_Fan();                  //Turn on fan
            INT1_flag = 0;                  //Clear the INT1 flag
        ...
        {
            Monitor_Fan();                  //Print fan information on teraterm
        }
        else                                //If it is not, then don't show fan and wait 1 second
        {        
            Wait_Half_Second();             //Do nothing for 1 second
            Wait_Half_Second();
        }
    }
