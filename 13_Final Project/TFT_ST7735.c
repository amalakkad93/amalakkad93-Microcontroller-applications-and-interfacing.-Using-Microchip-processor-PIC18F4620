#include <string.h>                         // Include the following libraries to make program work
#include <stdlib.h>
#include <xc.h>
#include "TFT_ST7735.h"
#include "Main.h"

void delay_ms(int ms)                       // Function to create a delay of variable length
{
           
#define COUNT_PER_MS    _XTAL_FREQ/4000     // Count per ms = 2000             
#define COUNT_SCALED    COUNT_PER_MS/8      // Scaled count

    int count;                              // Store count
    count = COUNT_SCALED * ms;              // Get the count to scale
    count = (0xffff - count);               // Find the value of where to start the count

	T3CON = 0xb8;                           // Timer 3, 16-bit mode, pre-scaler 1:8
...
const char font[] =                         // cont char array for the font
{
...
const char font2[] =                        // const char array for font2
{
...
void spiwrite(unsigned char spidata)        // Function for SPI Write
{
char x;
   PIR1bits.SSPIF = 0;  
   x = SSPBUF;                              // dummy read 
...
void write_command(unsigned char cmd_)      // Function for write cmd
{
    TFT_CS = 0;                             // Set TFT_CS, and DC to 0
    TFT_DC = 0;
    spiwrite(cmd_);                         // Call spiwrite function
 
    TFT_CS = 1;                             // TFT_CS to 1

}

void write_data(unsigned char data_)        // Function to now write data on display
{
    TFT_CS = 0;
    TFT_DC = 1;
    spiwrite(data_);
...
void LCD_Reset(void)                        // Function to reset the LCD
{
    TFT_RST = 0;
    delay_ms(100);
    TFT_RST = 1;
